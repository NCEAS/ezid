...
//            HttpClientContext localcontext = HttpClientContext.create();
    ...
    private byte[] sendRequest(int requestType, String uri, String requestBody) throws EZIDException {
        ...
        // DataCite EZID API implementation does not support session cookie
        // so we force Authorization header for each request
        // see https://support.datacite.org/docs/datacite-ez-api-guide#authentication
        Header[] headers = localcontext.getRequest().getHeaders("Authorization");
        String[] BAvalue = headers[0].toString().split(":");
        String authHeader = BAvalue[1].trim();
    ...
    private String parseIdentifierResponse(String responseMsg) throws EZIDException {
        ...
        if (resultCode.equals(InternalProfile.SUCCESS.toString())) {
            if (serviceBaseUrl.contains("datacite") ){
                idList = (unescape(responseArray[1]).trim()).split("\n");
            } else {
                idList = (unescape(responseArray[1]).trim()).split("\\|");
            }
            newId = idList[0].trim();
